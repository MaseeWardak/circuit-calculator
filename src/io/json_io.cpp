...
// It only handles the specific JSON shapes described in json_io.hpp.
// ---------------------------------------------------------------------------
...
static double parse_number(const char* s, int& i) {
    ...
    if (s + i == start) {
        throw circuit_error("JSON parse: expected a number");
    }
    char buf[64];
    const int len = static_cast<int>(s + i - start);
    if (len >= 64) {
        throw circuit_error("JSON parse: number token too long");
    }
    for (int j = 0; j < len; ++j) {
        buf[j] = start[j];
    }
...
ParsedCircuit parse_netlist_json(const char* json) {
    ...
    while (json[i] && json[i] != '}') {
        ...
        if (key == "node_count") {
            ...
            if (node_count < 1) {
                throw circuit_error("node_count must be at least 1");
            }
    ...
    if (node_count == 0) {
        throw circuit_error("missing node_count field");
    }
    return out;
}
...
static std::string dbl_to_str(double v) {
    // Enough precision for any circuit value we'd see in practice.
    char buf[32];
...
std::string error_to_json(const std::string& msg) {
    // Escape the few characters that could break JSON.
    std::string safe;
